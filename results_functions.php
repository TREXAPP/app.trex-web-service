...
function DrawRaceInfoTable($RaceID, $StartTime, $dateSeparator) {
	...
	- running - formula: started - finished - dnf&dsq
	- finished - kolku trkaci imaat entry za Finish(posledna kontrolna)
	- dnf&dsq - kolku imaat vo ActiveRacers, status DNF ili DSQ
	...
	*/
	...
	//Registered Runners:
	$query = "SELECT Count(ActiveRacerID) AS Count FROM `ActiveRacers` WHERE Hide=0 AND RaceID='" . $RaceID . "';";
	...
	//			WHERE Hide = 0 AND Valid = 1 AND CPID = 8 AND RaceID = '" . $RaceID . "';";
				
	$query = "	SELECT Count(DISTINCT(CPEntries.BIB)) AS Count
				FROM `CPEntries`
				JOIN ActiveRacers ON CPEntries.BIB = ActiveRacers.BIB
				WHERE RaceID =" . $RaceID . " AND Valid = 1";
	...
	$query = "	SELECT Count(DISTINCT(CPEntries.BIB)) AS Count
				FROM `CPEntries`
				JOIN ActiveRacers ON CPEntries.BIB = ActiveRacers.BIB
				WHERE RaceID =" . $RaceID . " AND Valid = 1 AND CPNo = 99 AND Status='OK'";
	...
	if ($StartPassed) {
		$query = "	SELECT Count(ActiveRacerID) as Count FROM `ActiveRacers`
					WHERE Hide = 0 AND (Status = 'DNF' OR Status = 'DSQ') AND RaceID = '" . $RaceID . "';";
		$result = mysqli_query($conn,$query);
		if ($result->num_rows > 0) {
			$row = $result->fetch_array(MYSQL_ASSOC);
			$dnfdsq = $row['Count'];
	...
	//running
	if ($StartPassed) {
		$running = $started - $finished - $dnfdsq;
		if ($running < 0) {
			$running = 'N/A';
		}
	} else {
	...
	*/		
		...
		//$raceInfoHtml .= "<td class='race-info-table-cell-right' colspan=3>" . date_format(date_create($StartTime), 'l, d M Y, H:i:s\h') . " CET</td>";
			$raceInfoHtml .= '<span class="hideThis" id="StartTime_' . $RaceID . '">' . $StartTime . '</span>';
			$raceInfoHtml .= "<table class='race-info-table'><tbody>";
			$raceInfoHtml .= "<tr>";
			
			if (!$StartPassed) {
				$raceInfoHtml .= "<td class='race-info-table-cell-left countdown_label' colspan=1>Starts In:</td>";
				$raceInfoHtml .= "<td class='race-info-table-cell-right' colspan=3><span class='timer countdown' id='time_" . $RaceID . "'></span></td>";
			} else {
				if ($running > 0) {
					$raceInfoHtml .= "<td class='race-info-table-cell-left countup_label' colspan=1>Time passed:</td>";
					$raceInfoHtml .= "<td class='race-info-table-cell-right' colspan=3><span class='timer countup' id='time_" . $RaceID . "'></span></td>";
				} else {
					$raceInfoHtml .= "<td class='race-info-table-cell-left' colspan=1>Started At:</td>";
		...
		if ($StartPassed && ($running == 0)) {
			...
			$raceInfoHtml .= "<tr>";
			$raceInfoHtml .= "<td class='race-info-table-cell-left'>DNF/DSQ:</td>";
			$raceInfoHtml .= "<td class='race-info-table-cell-right'>" . $dnfdsq . "</td>";
...
function IsStartPassed($StartTime) {
	...
	if (strtotime($timeNow) > strtotime($StartTime)) {
		return true;
	} else {
		return false;
	}
...
function DrawResultsHeader($raceName) {
	...
	$returnStr .= "	<div class='header-headline-wrapper'><h2 class='headline'>" . $raceName . "<br>Official Results</h2></a></div>";
...
function getTableArray($headerIDsArray, $RaceGroupID, $RaceID, $StartTime) {
	...
	$q = "SELECT CPNo, ActiveRacers.BIB, Time, ActiveRacers.ActiveRacerID FROM `CPEntries`
		...
		WHERE Hide = 0 AND Valid = 1 AND RaceID = " . $RaceID;
	...
	$q = "SELECT ActiveRacerID, CONCAT(FirstName, ' ', LastName) AS Name, Gender, Country, BIB, DateOfBirth, Status, TeamName FROM `ActiveRacers`
			...
			WHERE Hide = 0 AND RaceID = " . $RaceID . " ORDER BY BIB";
	...
	foreach ($sortTotal as &$sortTotalRow) {
		...
		foreach ($tableArray as &$tableArrayRow) {
			...
			if (!$found && $sortTotalRow['BIB'] == $tableArrayRow[3]) {
				...
				if ($sortedTableArray[$sortedIndex][9] != 'DNF' && $sortedTableArray[$sortedIndex][9] != 'DSQ' && $sortedTableArray[$sortedIndex][10] != '--') {			
					//Position (total)
						$totalPos = $sortedIndex-$dnfdsqcount+1;			
					...
					$dnfdsqcount++;
					//Position (total)			
					$sortedTableArray[$sortedIndex][0] = '<span class="position-table-text">--</span>';
					//Position (gender)
					if ($sortedTableArray[$sortedIndex][5] == 'F') {
						$sortedTableArray[$sortedIndex][1] = 'F | <span class="position-table-text">--</span>';
					} else {
						$sortedTableArray[$sortedIndex][1] = 'M | <span class="position-table-text">--</span>';
					}
					
					//Position (Category)
					if (((int)$sortedTableArray[$sortedIndex][6] < 50) && ($sortedTableArray[$sortedIndex][5] == 'F')) {
						//F1
						$sortedTableArray[$sortedIndex][2] = 'F(18-50) | <span class="position-table-text">--</span>';
					}
					if (((int)$sortedTableArray[$sortedIndex][6] >= 50) && ($sortedTableArray[$sortedIndex][5] == 'F')) {
						//F2
						$sortedTableArray[$sortedIndex][2] = 'F(50+) | <span class="position-table-text">--</span>';
					}
					if (((int)$sortedTableArray[$sortedIndex][6] < 50) && ($sortedTableArray[$sortedIndex][5] == 'M')) {
						//M1
						$sortedTableArray[$sortedIndex][2] = 'M(18-50) | <span class="position-table-text">--</span>';
					}
					if (((int)$sortedTableArray[$sortedIndex][6] >= 50) && ($sortedTableArray[$sortedIndex][5] == 'M')) {
						//M2
						$sortedTableArray[$sortedIndex][2] = 'M(50+) | <span class="position-table-text">--</span>';
					}
		...
		while (isset($tableArray[$rowCounter][0])) {
			...
			while (isset($tableArray[$rowCounter][$dataCounter])) {
				...
				if (($dataCounter > 10) && ($dataCounter < count($tableArray[0])-1)) {				
					...
					if ($dataCounter == 4) {
						//
						//$returnStr .= TitleCase(cyr2lat($tableArray[$rowCounter][$dataCounter]));
						$returnStr .= TitleCase(cyr2lat($tableArray[$rowCounter][$dataCounter]));
					} else {
						//Team
						if ($dataCounter == 7) {
							$returnStr .= cyr2lat($tableArray[$rowCounter][$dataCounter]);
						} else {
							//$returnStr .= TitleCase(cyr2lat($tableArray[$rowCounter][$dataCounter]));
...
function TitleCase($string) {
	...
	$count = 0;
	$returnStr = '';
	while(isset($words[$count])) {
		if ($returnStr != '') {
			$returnStr .= ' ';
		}
		$returnStr .= ucfirst(strtolower($words[$count]));
		$count++;
	}
	return $returnStr;	
}
